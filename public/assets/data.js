...
{ id:'set1', nav:'1 · s a t p i n', c:'#3D7BFF', soft:'#E7F0FF', kind:'g', art:'cat',
...
/* ==================== ПОЧЕМУ ЗВУКИ НЕ ОЗВУЧЕНЫ ====================
   Здесь был список звуков, которые пробовали записать синтезом: 's' как
   'sssss', /ɒ/ как 'ohhhh' и так далее. Не работает и работать не будет.

   Синтез речи выдаёт слова, а не фонемы. Растянутая гласная для него —
   междометие, и он её пропевает: /ɒ/ вышло спетым «о-о-о», /iː/ было не
   отличить от названия буквы u, /m/ прозвучало как «мим». Дело не в модели
   и не в голосе — отдельный звук просто не является речью, и просить его у
   речевого синтеза бессмысленно.

   Взрывные /p/ /b/ /t/ /d/ /k/ /g/ невозможны отдельно и физически: без
